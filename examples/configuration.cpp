 ...
 * @file
 *
 * This is an example of how to create and use configuration objects in
 * JayBeams.
 */
...
public:
  ...
  // names, if you want you can set the default value ...
  thread_config()
      : name(desc("name"), this, "")
...
public:
  ...
  // message, as well as a "class", allowing you to change all the
  // attributes with that class in a single configuration section
  // (more on this later) ...
  worker_config()
      : cl_device_name(
            desc("cl-device-name")
                .help("The name of the CL device used by this worker"),
            this)
...
int main(int argc, char* argv[]) try {
  ...
  for (auto w : config.workers()) {
    std::cout << "worker." << cnt << ".cl-device-name = " << w.cl_device_name()
              << std::endl;
    cnt++;
  }
