...
private:
  ...
  config_attribute(config_attribute<container_type, value_type> const&) =
      delete;

  config_attribute&
  operator=(config_attribute<container_type, value_type>&& rhs) {
    value_ = std::move(rhs.value_);
    return *this;
  }
  config_attribute&
  operator=(config_attribute<container_type, value_type> const& rhs) {
    value_ = rhs.value_;
    return *this;
  }
  //@}
