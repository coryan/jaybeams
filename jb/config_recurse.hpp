...
struct config_recurse {
  ...
  typename std::enable_if<std::is_base_of<config_object,T>::value, void>::type
  apply_overrides(
      T& lhs, YAML::Node const& by_name, class_overrides const& by_class) {
  ...
  typename std::enable_if<
    std::is_base_of<
      config_object, typename Sequence::value_type>::value,void>::type
  apply_overrides(
  ...
  void add_options(
      ...
      config_object::attribute_descriptor const& d, Args...) {
    options.add_options()
   ...
   */
  template<typename C, typename T> static
  void validate(config_attribute<C,T> const& object);
   ...
   */
  template<typename U, typename V> static
  void validate(std::pair<U,V> const& object) {
  ...
  template<typename T, typename... Args> static
  YAML::Node to_yaml(T const& x, Args...) {
    YAML::Node doc;
    doc = x;
    return doc;
  }
   ...
   */
  template<typename C, typename T> static
  YAML::Node to_yaml(config_attribute<C,T> const& object);
  ...
  typename std::enable_if<
    std::is_base_of<
      config_object, typename Sequence::value_type>::value,YAML::Node>::type
   ...
   */
  template<typename U, typename V> static
  YAML::Node to_yaml(std::pair<U,V> const& object) {
    YAML::Node doc;
