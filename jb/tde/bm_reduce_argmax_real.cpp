...
std::size_t gpu_argmax(
  ...
  return std::distance(
      dev.begin(), boost::compute::max_element(
          dev.begin(), dev.end(), less_real, queue));
}

template<typename T>
std::size_t gpu_argmax_complex(
...
template<typename T>
std::size_t cpu_argmax(
    std::vector<std::complex<T>> const& host) {
  typedef std::complex<T> value_type;
...
template<typename precision_t, bool use_gpu = false>
class fixture {
 public:
  fixture(
      ...
      , unused(0)
  {
    ...
    boost::compute::copy(
        host.begin(), host.end(), dev.begin(), queue);
  }

  void run() {
    if (use_gpu) {
      unused += gpu_argmax_complex(dev, queue);
    } else {
 ...
 private:
  boost::compute::vector<std::complex<precision_t>> dev;
  std::vector<std::complex<precision_t>> host;
  boost::compute::command_queue queue;
  std::size_t unused;
};

template<typename precision_t, bool use_gpu>
void benchmark_test_case(config const& cfg) {
  ...
  typedef jb::testing::microbenchmark<fixture<precision_t, use_gpu>> benchmark;
