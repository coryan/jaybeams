...
class base_cache_prices {
  ...
  auto end() {
    return static_cast<side*>(this)->do_end();
  }
  auto begin() {
    return static_cast<side*>(this)->do_begin();
  }
  /**
  ...
  auto size() const {
    return static_cast<side*>(this)->do_size();
  }
  auto rbegin() {
    return static_cast<side*>(this)->do_rbegin();
  }
  auto empty() const {
    return static_cast<side*>(this)->do_empty();
  }
};
...
public:
  ...
  auto do_find_tail(price4_t px) {
    return tail_.find(px);
  }
  /**
   * @todo check limits, addapt return to make return types consistent
   * etc..
   */
  auto do_find_head(price4_t px) {
    return head_[px.as_integer()];
  }
