...
jb::itch5::compute_book_cache_aware::compute_book_cache_aware(
...
void jb::itch5::compute_book_cache_aware::handle_message(
  ...
  books_.emplace(msg.stock, order_book_cache_aware());
}
...
void jb::itch5::compute_book_cache_aware::handle_message(
  ...
  try {
    ...
    // new order ...
    order_data const& copy = std::get<2>(result_reduce);
...
void jb::itch5::compute_book_cache_aware::handle_unknown(
...
jb::itch5::compute_book_cache_aware::update_result
jb::itch5::compute_book_cache_aware::handle_add_no_update(
    time_point recv_ts, long msgcnt, std::size_t msgoffset,
    add_order_message const& msg) {
  ...
  if (stock_book_it == books_.end()) {
    auto book_pair = books_.emplace(msg.stock, order_book_cache_aware());
    stock_book_it = book_pair.first; // iterator to the new order book
  }
  // ... add the order to the book
  auto ru = stock_book_it->second.handle_add_order(
      msg.buy_sell_indicator, msg.price, msg.shares);
  return std::make_tuple(
      std::get<0>(ru), std::get<1>(ru), position.first->second);
}

void jb::itch5::compute_book_cache_aware::handle_reduce(
    time_point recv_ts, long msgcnt, std::size_t msgoffset,
...
jb::itch5::compute_book_cache_aware::update_result
jb::itch5::compute_book_cache_aware::handle_reduce_no_update(
    time_point recv_ts, long msgcnt, std::size_t msgoffset,
    message_header const& header, std::uint64_t order_reference_number,
    std::uint32_t shares, bool all_shares) {
  ...
  auto ru = stock_book_it->second.handle_order_reduced(
      copy.buy_sell_indicator, copy.px, shares);
  return std::make_tuple(std::get<0>(ru), std::get<1>(ru), copy);
}
