## Results for `r basename(data.filename)`
...
```{r fig.height=svg.h, fig.width=svg.w, dev='svg'}
...
```{r fig.height=svg.h, fig.width=svg.w, dev='png'}
ggplot(data=data,
...
That is, we need at least `r round(desired.nsamples, 2)`
...
if (desired.nsamples > length(data.array.ts)) {
    warning("Not enough samples in 'array' data to",
            ...
            min.delta, ") should be >=", desired.nsamples,
