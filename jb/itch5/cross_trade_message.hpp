...
struct decoder<V, cross_trade_message> {
  ...
  r(std::size_t size, void const* buf, std::size_t off) {
    ...
    x.stock = decoder<V, stock_t>::r(size, buf, off + 19);
    x.cross_price = decoder<V, price4_t>::r(size, buf, off + 27);
    x.match_number = decoder<V, std::uint64_t>::r(size, buf, off + 31);
    x.cross_type = decoder<V, cross_type_t>::r(size, buf, off + 39);
    return x;
  }
};
