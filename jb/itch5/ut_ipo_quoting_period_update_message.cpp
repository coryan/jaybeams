...
BOOST_AUTO_TEST_CASE(decode_ipo_quoting_period_update_message) {
  ...
  auto x = decoder<true, ipo_quoting_period_update_message>::r(buf.second,
                                                               buf.first, 0);
  BOOST_CHECK_EQUAL(x.header.message_type,
                    ipo_quoting_period_update_message::message_type);
  ...
  BOOST_CHECK_EQUAL(x.ipo_quotation_release_time.seconds().count(),
                    expected_release.count());
  BOOST_CHECK_EQUAL(x.ipo_quotation_release_qualifier, u'A');
  BOOST_CHECK_EQUAL(x.ipo_price, price4_t(1230500));

  x = decoder<false, ipo_quoting_period_update_message>::r(buf.second,
                                                           buf.first, 0);
  BOOST_CHECK_EQUAL(x.header.message_type,
                    ipo_quoting_period_update_message::message_type);
  BOOST_CHECK_EQUAL(x.header.stock_locate, 0);
...
BOOST_AUTO_TEST_CASE(stream_ipo_quoting_period_update_message) {
  ...
  auto buf = jb::itch5::testing::ipo_quoting_period_update();
  auto tmp = decoder<false, ipo_quoting_period_update_message>::r(buf.second,
                                                                  buf.first, 0);
  std::ostringstream os;
  os << tmp;
  BOOST_CHECK_EQUAL(os.str(), "message_type=K,stock_locate=0"
...
BOOST_AUTO_TEST_CASE(simple_ipo_quotation_release_qualifier) {
  ...
  BOOST_CHECK_THROW(ipo_quotation_release_qualifier_t(u'*'),
                    std::runtime_error);
}
