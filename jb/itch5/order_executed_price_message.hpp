...
struct order_executed_price_message : public order_executed_message {
  ...
  order_executed_price_message&
  operator=(order_executed_price_message const&) = default;
  order_executed_price_message&
  operator=(order_executed_price_message&&) = default;
...
struct decoder<V, order_executed_price_message> {
  ...
  r(std::size_t size, void const* buf, std::size_t off) {
    ...
    x.execution_price = decoder<V, price4_t>::r(size, buf, off + 32);
    return x;
  }
};
