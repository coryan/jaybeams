...
struct decoder<V, net_order_imbalance_indicator_message> {
  ...
  r(std::size_t size, void const* buf, std::size_t off) {
    ...
    x.paired_shares = decoder<V, std::uint64_t>::r(size, buf, off + 11);
    x.imbalance_shares = decoder<V, std::uint64_t>::r(size, buf, off + 19);
    x.imbalance_direction =
        decoder<V, imbalance_direction_t>::r(size, buf, off + 27);
    x.stock = decoder<V, stock_t>::r(size, buf, off + 28);
    x.far_price = decoder<V, price4_t>::r(size, buf, off + 36);
    x.near_price = decoder<V, price4_t>::r(size, buf, off + 40);
    x.current_reference_price = decoder<V, price4_t>::r(size, buf, off + 44);
    x.cross_type = decoder<V, cross_type_t>::r(size, buf, off + 48);
