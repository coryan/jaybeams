...
udp_config_common::udp_config_common()
    : debug(
          desc("debug").help(
              "Set the SO_DEBUG option on the socket.  On "
    ...
    , linger_seconds(
          desc("linger-seconds")
              .help(
                  "Set the timeout value for the SO_LINGER "
    ...
    , receive_buffer_size(
          desc("receive-buffer-size")
              .help(
                  "Set the SO_RCVBUF option -- the maximum socket receive "
    ...
    , receive_low_watermark(
          desc("receive-low-watermark")
              .help(
                  "Set the SO_RCVLOWAT option -- the minimum number of bytes "
    ...
    , send_buffer_size(
          desc("send-buffer-size")
              .help(
                  "Set the SO_SNDBUF option -- the maximum socket send "
                  "buffer.  By default, or if set to -1, use the system "
                  "defaults."),
          this, -1)
    , send_low_watermark(
          desc("send-low-watermark")
              .help(
                  "Set the SO_SNDLOWAT option -- the minimum number of bytes "
