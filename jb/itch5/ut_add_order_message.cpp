...
BOOST_AUTO_TEST_CASE(decode_add_order_message) {
  ...
  auto x = decoder<true,add_order_message>::r(bufsize, buf, 0);
  BOOST_CHECK_EQUAL(
      x.header.message_type, add_order_message::message_type);
  ...
  x = decoder<false,add_order_message>::r(bufsize, buf, 0);
  BOOST_CHECK_EQUAL(
      x.header.message_type, add_order_message::message_type);
  BOOST_CHECK_EQUAL(x.header.stock_locate, 0);
  BOOST_CHECK_EQUAL(x.header.tracking_number, 1);
...
BOOST_AUTO_TEST_CASE(stream_add_order_message) {
  ...
  auto tmp = decoder<false,add_order_message>::r(bufsize, buf, 0);
  std::ostringstream os;
  os << tmp;
  BOOST_CHECK_EQUAL(
      os.str(), "message_type=A,stock_locate=0"
