...
/// and from complex to floating point.
template <typename precision_t>
void test_plan_real2complex() {
  ...
  // contain the same contents as the input after a FFT and inverse
  // FFT transformation ...
  boost::multi_array<precision_t, 3> in(boost::extents[F][S][nsamples]);
  boost::multi_array<complex, 3> tmp(boost::extents[F][S][nsamples]);
  boost::multi_array<precision_t, 3> out(boost::extents[F][S][nsamples]);
...
/**
 * @test Verify that we can create and execute plans that convert
 * arrays of std::complex<float> to arrays of std::complex<float>.
 */
 ...
 * jb::fftw::create_backward_plan() detect errors correctly for
 * double.
 */
...
/**
 * @test Verify that we can create and execute plans that convert
 * arrays of double to arrays of std::complex<double> and vice-versa.
 */
...
/**
 * @test Verify that we can create and execute plans that convert
 * arrays of float to arrays of std::complex<float> and vice-versa.
 */
