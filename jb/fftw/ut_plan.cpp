 ...
 * @test Verify that we can create fftw_plan.
 */
BOOST_AUTO_TEST_CASE(fftw_plan_std_complex_double) {
  int nsamples = 1<<15;
  int tol = nsamples;
  ...
  complex* in = static_cast<complex*>(
      tested::traits::allocate(nsamples * sizeof(complex)));
  complex* tmp = static_cast<complex*>(
      tested::traits::allocate(nsamples * sizeof(complex)));
  complex* out = static_cast<complex*>(
      tested::traits::allocate(nsamples * sizeof(complex)));
