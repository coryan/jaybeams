...
void check_constructor() {
  BOOST_CHECK_THROW(
      jb::integer_range_binning<sample_type_t>(10, 10), std::exception);
  BOOST_CHECK_THROW(
      jb::integer_range_binning<sample_type_t>(20, 10), std::exception);
  BOOST_CHECK_NO_THROW(
      jb::integer_range_binning<sample_type_t>(1, 2));
  BOOST_CHECK_NO_THROW(
      jb::integer_range_binning<sample_type_t>(1000, 2000));
}
...
void check_basic() {
  ...
  BOOST_CHECK_EQUAL(
      bin.theoretical_min(), std::numeric_limits<sample_type_t>::min());
  BOOST_CHECK_EQUAL(
      bin.theoretical_max(), std::numeric_limits<sample_type_t>::max());
