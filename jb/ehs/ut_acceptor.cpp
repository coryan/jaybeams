 ...
 */
BOOST_AUTO_TEST_CASE(acceptor_accept_errors) {
  // Let the operating system pick a listening address ...
  boost::asio::ip::tcp::endpoint ep{boost::asio::ip::address_v4(), 0};
  ...
  std::thread t([&io_service]() {
    try {
      ...
      BOOST_FAIL("std::exception in accept thread: " << ex.what());
    } catch (...) {
      BOOST_FAIL("unknown exception in accept thread");
    }
  });
  ...
  // ... create a separate io service for the client side ...
  boost::asio::io_service io;
  // ... wait for the other thread to succeed, make sure we can create
  // at least one connection ...
  boost::asio::ip::tcp::socket sentinel{io};
  sentinel.connect(listen);
  for (int i = 0; i != 100 and dispatcher->get_accept_ok() == 0; ++i) {
    std::this_thread::sleep_for(std::chrono::milliseconds(10));
  }
  sentinel.close();
  BOOST_TEST_CHECKPOINT("sentinel");
  BOOST_TEST_MESSAGE("sentinel closed, io thread running");
  ...
  // ... this test is a bit flaky, it is basically trying to produce a
  // race between this thread and the accepting thread to force
  // execution of rare error paths in the accepting thread.  We try
  // multiple times, with different number of sockets, until there are
  // enough pending sockets that we have high confidence in the error ...
  for (int nsockets : {50, 100, 200, 400, 500, 600, 800}) {
    BOOST_TEST_MESSAGE("opening " << nsockets << " sockets");

    long initial = dispatcher->get_accept_ok();
    for (std::size_t i = 0; i != nsockets; ++i) {
      BOOST_TEST_CHECKPOINT("opening " << i << "/" << nsockets << " sockets");
      auto sock = std::make_shared<socket_t>(io);
      boost::system::error_code ec;
      sock->connect(listen, ec);
      if (ec) {
        continue;
      }
      sockets.push_back(sock);
    }
    if (dispatcher->get_accept_error() > 0) {
      // ... success, we triggered an error, break this loop ...
      break;
    }
    // ... if there are less than nsockets / 2 connections made we
    // attempt to trigger accept errors, otherwise try with more
    // sockets ...
    if (dispatcher->get_accept_ok() - initial < nsockets / 2) {
      BOOST_TEST_MESSAGE("shutdown acceptor at nsockets = " << nsockets);
      BOOST_TEST_CHECKPOINT("shutdown at " << nsockets);
      acceptor.shutdown();
      break;
    }
  ...
  // ... shutdown everything ...
  BOOST_TEST_CHECKPOINT("stop + join");
  io_service.dispatch([&io_service] {
    io_service.stop();
  });
  t.join();
  BOOST_TEST_CHECKPOINT("closing sockets");
  for (auto sock : sockets) {
    sock->close();
  }
  sockets.clear();
